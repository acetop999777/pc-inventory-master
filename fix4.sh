...
# 1) 统一 inventory query：把后端返回的 quantity/cost 强制转成 number（解决 +1 拼接/按钮无效）
...
# 2) inventory write-behind：写入成功后用后端返回 row 直接更新 cache（不依赖 invalidate 才能看到）
  ...
  const update = (id: string, fields: Partial<InventoryItem>) => {
    ...
    void queue.enqueue<InventoryWrite>({
      ...
      write: async (w) => {
        ...
        // 用后端返回的 updated row 立即对齐 cache（不用刷新）
...
# 3) InventoryHub：- 弹 confirm；+ 输入 unit cost + add qty，然后实时计算 WAC 并更新 quantity/cost
    ...
    return (
      <input
        ...
        onChange={(e) => {
          ...
          setTempVal(next);
          commit(next); // optimistic immediately; savequeue debounces actual write
        }}
...
# 4) AppLegacy：InventoryHub / InboundHub 不再传 inventory props（避免双数据源导致“改了但页面不变”）
python3 - <<'PY'
...
# 5) InboundHub：也改成用 useInventoryQuery（保持一致的单一数据源）
...
export default function InboundHub() {
  ...
  const handleCommit = async () => {
    ...
    batch.forEach((item) => {
      ...
      const inQty = Number(item.qtyInput ?? 0);
      const inCost = Number(item.costInput ?? 0);
...
git add -A
git commit -m "phase4.2: inventory realtime cache sync + qty confirm + WAC increment + normalize numbers + single source"
git tag -a phase4_2-$(date +%Y%m%d) -m "phase4.2: inventory realtime + WAC + qty controls"
