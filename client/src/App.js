...
import { 
  ...
  Plus, Minus, Save, X, Truck, ChevronLeft, Box, Cpu, Scan, Trash2, ArrowRight, 
  Calendar, Link as LinkIcon, User, TrendingUp, CreditCard, MapPin, Filter, ArrowDown, DollarSign
} from 'lucide-react';

/**
 * Project: PC Inventory Master
 * Version: 5.4.0 "Financial Real-time Core"
 */
...
const guessCategory = (name) => {
    ...
    if(n.includes('cpu')||n.includes('ryzen')) return 'CPU';
    ...
    if(n.includes('video')||n.includes('geforce')||n.includes('rtx')) return 'Video Card';
    if(n.includes('ssd')||n.includes('nvme')) return 'Storage';
    if(n.includes('cooler')||n.includes('liquid')) return 'CPU Cooler';
    if(n.includes('supply')||n.includes('psu')) return 'Power Supply';
...
const ClientHub = ({ data, refresh, notify, log }) => {
    ...
    const financialData = useMemo(() => {
        ...
        // 1. Calculate Real-time Cost from Specs
        let totalCost = 0;
        Object.values(active.specs || {}).forEach(item => {
            totalCost += (parseFloat(item.cost) || 0);
        });
        
        // 2. Calculate Profit based on Input Sale Price
        const salePrice = parseFloat(active.totalPrice) || 0;
        const netProfit = salePrice - totalCost;

        return { cost: totalCost, profit: netProfit };
    }, [active]); // Re-runs whenever 'active' state changes (including spec updates)

    const filtered = useMemo(() => {
        let list = data.clients.filter(c => {
            const matchSearch = JSON.stringify(c).toLowerCase().includes(search.toLowerCase());
            const matchDate = (!dateStart || c.orderDate >= dateStart) && (!dateEnd || c.orderDate <= dateEnd);
            return matchSearch && matchDate;
        });
        if(sortProfit) list.sort((a,b) => b.profit - a.profit);
        return list;
    }, [data.clients, search, sortProfit, dateStart, dateEnd]);

    const stats = useMemo(() => ({
        count: filtered.length,
        profit: filtered.reduce((a,b) => a + (b.profit||0), 0)
    }), [filtered]);

    const handleNew = () => {
        const initSpecs = {};
        SPEC_CATS.forEach(c => {
            if(c === 'Labor') initSpecs[c] = { name: 'Assembly Service', sku: 'LABOR', cost: 0, price: 180, qty: 1 };
            else if(c !== 'Custom' && c !== 'Strimer') initSpecs[c] = { name: '', sku: '', cost: 0, qty: 1 };
        });
        setActive({ 
    ...
    const save = async () => {
        const { cost, profit } = financialData; // Use the calculated values
    ...
    const displayCats = useMemo(() => {
        if(!active) return [];
        return SPEC_CATS.filter(c => 
            ['CPU', 'CPU Cooler', 'Motherboard', 'Memory', 'Storage', 'Video Card', 'Case', 'Power Supply', 'Labor'].includes(c) || 
            (active.specs[c] && active.specs[c].name)
        );
    }, [active]);
    ...
    if(view === 'detail') return (
        <div className="p-8 max-w-7xl mx-auto pb-32 animate-in slide-in-from-right">
            ...
            <div className="flex justify-between items-center mb-6">
                <button onClick={()=>setView('list')} className="text-slate-400 font-bold text-xs flex items-center gap-1 hover:text-slate-600"><ChevronLeft size={16}/> BACK</button>
                <div className="flex gap-3">
                    {active.pcppLink && <a href={active.pcppLink} target="_blank" className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2"><LinkIcon size={14}/> PCPartPicker</a>}
                    {active.trackingNumber && <a href={`https://www.ups.com/track?tracknum=${active.trackingNumber}`} target="_blank" className="bg-amber-50 text-amber-700 px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2"><Truck size={14}/> Track Package</a>}
                    <button onClick={save} className="bg-slate-900 text-white px-8 py-2 rounded-xl font-bold text-xs uppercase shadow-lg">Save Profile</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                {/* Right: Specs & Financials */}
                <div className="xl:col-span-2 space-y-6">
                    {/* FINANCIAL STRIP (New Elegant Design) */}
                    <div className="grid grid-cols-3 gap-4">
                        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1"><DollarSign size={12}/> Total Sale Price</label>
                            <input className="text-2xl font-black text-slate-800 bg-transparent outline-none w-full placeholder:text-slate-200" placeholder="0.00" value={active.totalPrice||''} onChange={e=>setActive({...active, totalPrice:parseFloat(e.target.value)})} />
                        </div>
                        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200/50 flex flex-col justify-between">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Cost (Auto)</label>
                            <div className="text-2xl font-black text-slate-500">{formatMoney(financialData.cost)}</div>
                        </div>
                        <div className={`p-5 rounded-2xl border flex flex-col justify-between ${financialData.profit >= 0 ? 'bg-emerald-50/50 border-emerald-100' : 'bg-red-50/50 border-red-100'}`}>
                            <label className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${financialData.profit >= 0 ? 'text-emerald-500' : 'text-red-400'}`}>Net Profit</label>
                            <div className={`text-2xl font-black ${financialData.profit >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{formatMoney(financialData.profit)}</div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                        ...
                        <div className="space-y-2">
                            <div className="grid grid-cols-12 gap-4 px-4 pb-2 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                <div className="col-span-2">Type</div><div className="col-span-6">Model</div><div className="col-span-2">SKU</div><div className="col-span-2 text-right">Cost</div>
                            </div>
                            {displayCats.map(cat => {
        ...
        <div className="p-8 max-w-7xl mx-auto">
             <div className="flex gap-4 mb-6 items-center">
                 <div className="flex-1 bg-white p-2 rounded-xl border border-slate-100 flex items-center gap-2">
                     ...
                     <input className="w-full text-xs font-bold outline-none" placeholder="Search Clients, Real Name, Parts..." value={search} onChange={e=>setSearch(e.target.value)}/>
                 </div>
                 <div className="flex items-center gap-2 bg-white p-2 rounded-xl border border-slate-100">
                     <input type="date" className="text-[10px] font-bold outline-none bg-transparent" value={dateStart} onChange={e=>setDateStart(e.target.value)}/>
                     <span className="text-slate-300">-</span>
                     <input type="date" className="text-[10px] font-bold outline-none bg-transparent" value={dateEnd} onChange={e=>setDateEnd(e.target.value)}/>
                 </div>
                 <button onClick={()=>setSortProfit(!sortProfit)} className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-1 border ${sortProfit ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-white border-slate-100 text-slate-500'}`}><TrendingUp size={14}/> Profit</button>
             ...
             {/* Stats Bar */}
             <div className="mb-6 flex gap-6 px-2">
                 <div><div className="text-[10px] font-bold text-slate-400 uppercase">Filtered Clients</div><div className="text-xl font-black">{stats.count}</div></div>
                 <div><div className="text-[10px] font-bold text-slate-400 uppercase">Total Profit</div><div className="text-xl font-black text-emerald-600">{formatMoney(stats.profit)}</div></div>
             </div>

             <div className="grid grid-cols-12 gap-4 p-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                 <div className="col-span-3">Client</div>
                 <div className="col-span-2 text-right">Sale / Cost</div>
                 <div className="col-span-1 text-right">Profit</div>
                 <div className="col-span-2 text-center">Timeline</div>
                 <div className="col-span-1 text-center">Labor</div>
                 <div className="col-span-3">Address</div>
             </div>

             <div className="space-y-2">
                 {filtered.map(c => (
                     <div key={c.id} onClick={()=>{setActive(c); setView('detail');}} className="grid grid-cols-12 gap-4 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm items-center hover:border-blue-200 cursor-pointer transition-all group">
                         <div className="col-span-3">
                             <div className="font-black text-sm text-slate-800">{c.wechatName}</div>
                             <div className="text-[10px] font-bold text-slate-400">{c.realName || '-'}</div>
                         </div>
                         <div className="col-span-2 text-right">
                             <div className="font-bold text-xs">{formatMoney(c.totalPrice)}</div>
                             <div className="text-[9px] text-slate-400">Cost: {formatMoney(c.actualCost)}</div>
                         </div>
                         <div className="col-span-1 text-right">
                             <div className="font-black text-xs text-emerald-600">+{formatMoney(c.profit)}</div>
                         </div>
                         <div className="col-span-2 text-center">
                             <div className="text-[9px] font-bold text-slate-500">{c.orderDate ? new Date(c.orderDate).toLocaleDateString() : '-'}</div>
                             <div className="text-[9px] text-slate-300">to</div>
                             <div className="text-[9px] font-bold text-slate-500">{c.deliveryDate ? new Date(c.deliveryDate).toLocaleDateString() : '-'}</div>
                         </div>
                         <div className="col-span-1 text-center">
                             <div className="bg-slate-50 rounded-lg py-1 text-[10px] font-bold text-slate-500">-</div>
                         </div>
                         <div className="col-span-3 text-[10px] text-slate-500 truncate font-medium">
                             {c.address ? `${c.address}, ${c.city} ${c.zip}` : 'No Address'}
                         </div>
                     </div>
                 ))}
             </div>
...
const StockVault = ({ data, refresh, notify, log }) => {
    ...
    const commitAdd = async () => {
        const newQty = editItem.quantity + adjQty;
        const newCost = ((editItem.quantity * editItem.cost) + (adjQty * adjCost)) / newQty;
        await apiCall('/inventory/batch', 'POST', [{ ...editItem, quantity: newQty, cost: parseFloat(newCost.toFixed(2)) }]);
        notify('Stock Added'); setEditItem(null); refresh();
    };

    const commitSub = async () => {
        if(editItem.quantity < adjQty) { notify('Error: Insufficient Stock', 'error'); return; }
        const newQty = editItem.quantity - adjQty;
        await apiCall('/inventory/batch', 'POST', [{ ...editItem, quantity: newQty, cost: editItem.cost }]);
        log('STOCK_OUT', 'Stock Removed', `${editItem.name} -${adjQty}`);
        notify('Stock Removed'); setEditItem(null); setConfirmOut(false); refresh();
    };
        ...
        <div className="p-8 max-w-5xl mx-auto">
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
                <div className="grid grid-cols-12 gap-4 p-4 bg-slate-50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    <div className="col-span-1">Cat</div><div className="col-span-5">Product</div><div className="col-span-2">SKU / Keyword</div><div className="col-span-2 text-right">Stock</div><div className="col-span-2 text-right">Action</div>
                </div>
                {filtered.map(i => (
                    <div key={i.id} className="grid grid-cols-12 gap-4 p-4 border-b border-slate-50 hover:bg-slate-50 items-center">
                        ...
                        <div className="col-span-5 font-bold text-sm text-slate-800 truncate">{i.name}</div>
                        <div className="col-span-2 text-xs font-mono text-slate-400">
                            <span className="bg-blue-50 text-blue-600 px-1 rounded mr-1">{i.keyword}</span>
                            {i.sku}
                        </div>
                        ...
                        <div className="col-span-2 flex justify-end gap-2"><button onClick={()=>{setEditItem(i);setAdjQty(1);setAdjCost(i.cost);setConfirmOut(false);}} className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-100 text-slate-400"><Plus size={16}/></button></div>
                    </div>
                ))}
            </div>
            ...
            {editItem && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[70] flex items-center justify-center p-6">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95">
                        <h3 className="text-lg font-black text-slate-800 mb-6">{confirmOut ? 'Confirm Stock Out' : 'Adjust Stock'}</h3>
                        ...
                        {!confirmOut ? (
                            <>
                                <div className="grid grid-cols-2 gap-4 mb-6">
                                    <div><label className="l">Qty Change</label><input type="number" className="i text-lg h-12" value={adjQty} onChange={e=>setAdjQty(parseInt(e.target.value)||0)}/></div>
                                ...
                                <div className="mb-6 flex justify-between items-center bg-blue-50 p-4 rounded-xl border border-blue-100">
                                     <div><div className="text-[9px] font-bold text-blue-400 uppercase">New Avg Cost (Add)</div><div className="text-xl font-black text-blue-700">{formatMoney( (editItem.quantity*editItem.cost + adjQty*adjCost)/(editItem.quantity+adjQty) )}</div></div>
                                </div>
                                <div className="flex gap-3">
                                     <button onClick={()=>setConfirmOut(true)} className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 py-4 rounded-2xl font-bold text-sm uppercase transition-colors">Stock Out</button>
                                     <button onClick={commitAdd} className="flex-1 bg-slate-900 hover:bg-slate-800 text-white py-4 rounded-2xl font-bold text-sm uppercase transition-colors shadow-lg">Add Stock</button>
                                </div>
                            </>
                        ) : (
                            <div className="space-y-4">
                                <div className="bg-red-50 p-4 rounded-2xl border border-red-100 text-center">
                                    <div className="text-xs font-bold text-red-400 uppercase mb-2">Removing</div>
                                    <div className="text-3xl font-black text-red-600">-{adjQty} Units</div>
                                    <div className="mt-2 text-xs font-bold text-red-400">Remaining Stock: {editItem.quantity - adjQty}</div>
                                </div>
                                <button onClick={commitSub} className="w-full bg-red-600 text-white py-4 rounded-2xl font-bold text-sm uppercase shadow-lg hover:bg-red-700">Confirm Removal</button>
                                <button onClick={()=>setConfirmOut(false)} className="w-full py-3 text-slate-400 font-bold text-xs uppercase">Back</button>
                            </div>
                        )}
                        
                        {!confirmOut && <button onClick={()=>setEditItem(null)} className="w-full mt-4 text-slate-400 font-bold text-xs uppercase hover:text-slate-600">Cancel</button>}
                    </div>
                </div>
            )}
        </div>
...
// --- INTAKE NODE (Code remains same as V5.2) ---
        ...
        try {
            ...
            for(let i=0; i<lines.length; i++) {
                if(lines[i].startsWith('Item #:')) {
                    ...
                    let subtotal = 0; let qty = 1; let foundPrice = false;
                    for(let j=1; j<8; j++) {
                        const l = lines[i+j];
                        if(l && l.includes('ea.)')) {
                    ...
                    if(!foundPrice) {
                        for(let j=1; j<6; j++) {
                            const l = lines[i+j];
                            if(l && l.startsWith('$') && !l.includes('ea.')) {
                                const qLine = lines[i+j-1];
                                if(qLine && /^\d+$/.test(qLine)) {
                                    subtotal = parseFloat(l.replace(/[$,]/g, ''));
                                    qty = parseInt(qLine);
                                    foundPrice = true; break;
                                }
                            }
            ...
            setNeweggTxt(''); notify(`Parsed ${items.length} items`);
            ...
            <div className="space-y-6 h-full flex flex-col">
                <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex-shrink-0">
                    ...
                    <form onSubmit={handleScan} className="relative">
                        ...
                        <input autoFocus className="w-full bg-slate-50 pl-12 pr-4 py-3.5 rounded-xl font-bold text-sm outline-none" placeholder="Scan SKU / UPC..." value={scanVal} onChange={e=>setScanVal(e.target.value)}/>
                    </form>
                </div>
                <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex-1 flex flex-col min-h-0">
                    ...
                    <textarea className="flex-1 bg-slate-50 rounded-xl p-4 text-xs font-mono mb-4 outline-none resize-none" placeholder="Paste Newegg Order Summary..." value={neweggTxt} onChange={e=>setNeweggTxt(e.target.value)}/>
                    <button onClick={parseNewegg} className="w-full bg-blue-50 text-blue-600 py-4 rounded-xl font-bold text-xs uppercase">Analyze & Extract</button>
                </div>
            </div>
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 flex flex-col h-full min-h-0">
                <div className="flex justify-between items-center mb-6 flex-shrink-0"><h3 className="font-black uppercase text-xs text-slate-400">Staging ({batch.length})</h3>{batch.length>0 && <button onClick={commit} className="bg-slate-900 text-white px-6 py-2 rounded-xl font-bold text-xs uppercase">Commit All</button>}</div>
                <div className="flex-1 overflow-y-auto space-y-4 pr-2 no-scrollbar">
                    {batch.map((item, i) => (
                        <div key={i} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3">
                            <div className="flex justify-between items-start"><input className="font-bold text-sm w-full bg-transparent outline-none" value={item.name} onChange={e=>{const n=[...batch];n[i].name=e.target.value;setBatch(n)}}/><button onClick={()=>setBatch(batch.filter((_,idx)=>idx!==i))}><Trash2 size={16} className="text-slate-300"/></button></div>
                            <div className="flex flex-wrap gap-2">
                                ...
                                <select className="text-[10px] bg-slate-50 rounded px-2 py-1 outline-none" value={item.category} onChange={e=>{const n=[...batch];n[i].category=e.target.value;setBatch(n)}}>{SPEC_CATS.map(c=><option key={c} value={c}>{c}</option>)}</select>
                            </div>
