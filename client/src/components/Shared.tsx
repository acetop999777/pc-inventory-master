...
export const InlineEdit: React.FC<InlineEditProps> = ({
  ...
  return (
    <div
      ...
      className={`cursor-pointer hover:bg-blue-50 px-1 rounded border border-transparent hover:border-blue-100 ${className}`}
    >
...
export const NavIcon: React.FC<NavIconProps> = ({ icon: I, active, onClick, label }) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center gap-1.5 transition-all group ${active ? 'text-blue-600 -translate-y-1' : 'text-slate-400 hover:text-slate-600'}`}
  >
    <div
      className={`p-2 rounded-xl transition-colors ${active ? 'bg-blue-50' : 'group-hover:bg-slate-50'}`}
    >
      <I size={24} strokeWidth={active ? 2.5 : 2} />
    </div>
