            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                {filtered.map(i => (
                    <div key={i.id} className="grid grid-cols-1 md:grid-cols-12 gap-2 px-4 py-2 border-b border-slate-50 hover:bg-slate-50/80 transition-colors items-center group">
                        ...
                        {/* 2. Name */}
                        <div className="md:col-span-5 font-bold text-[13px] text-slate-700 truncate pr-4" title={i.name}>
                        ...
                        <div className="hidden md:flex md:col-span-3 justify-end items-center gap-3">
                            ...
                            <div className="flex items-center bg-white border border-slate-200 rounded-md h-7 shadow-sm">
                                <button onClick={()=>handleQuickMinus(i)} className="w-6 h-full flex items-center justify-center text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-l-md transition-all active:bg-red-100"><Minus size={10} strokeWidth={4}/></button>
                                ...
                                <button onClick={()=>openAddModal(i)} className="w-6 h-full flex items-center justify-center text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-r-md transition-all active:bg-blue-100"><Plus size={10} strokeWidth={4}/></button>
                            </div>

                            {/* History & Delete */}
                            <div className="flex items-center gap-1 ml-1">
                                <button onClick={()=>handleViewHistory(i)} className="p-1.5 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-md transition-colors" title="View Audit Log">
                                    <History size={14}/>
                                </button>
                                <button onClick={()=>deleteItem(i)} className="p-1.5 text-slate-200 hover:text-red-400 hover:bg-red-50 rounded-md transition-colors">
                                    <Trash2 size={14}/>
                                </button>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in zoom-in-95">
                    <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-6">
                            <div><label className="text-[9px] font-black text-slate-400 uppercase mb-1.5 block ml-1">Qty Add</label><input type="number" className="w-full bg-slate-100 p-2.5 rounded-lg font-black text-lg outline-none focus:ring-2 focus:ring-blue-500 text-center" value={addQty} onChange={e=>setAddQty(parseInt(e.target.value)||0)} autoFocus onFocus={e=>e.target.select()}/></div>
                            <div><label className="text-[9px] font-black text-slate-400 uppercase mb-1.5 block ml-1">Unit Cost</label><input type="number" className="w-full bg-slate-100 p-2.5 rounded-lg font-black text-lg outline-none focus:ring-2 focus:ring-emerald-500 text-center text-emerald-600" value={addCost} onChange={e=>setAddCost(parseFloat(e.target.value)||0)} onFocus={e=>e.target.select()}/></div>
                        </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex justify-end animate-in fade-in">
                    <div className="bg-white w-full max-w-md h-full shadow-2xl p-6 flex flex-col animate-in slide-in-from-right duration-300">
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                <h3 className="font-black text-slate-800 text-lg flex items-center gap-2"><History size={20}/> Audit Log</h3>
                                <p className="text-xs text-slate-400 mt-1 font-mono">{historyItem.sku || 'NO-SKU'}</p>
                            </div>
                            <button onClick={()=>setHistoryItem(null)} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 transition-colors"><X size={18} className="text-slate-500"/></button>
                        </div>
                        
                        <div className="flex-1 overflow-y-auto no-scrollbar space-y-4">
                            {loadingLogs ? (
                                <div className="text-center py-10 text-slate-400 text-xs animate-pulse">Loading trace data...</div>
                            ) : historyLogs.length === 0 ? (
                                <div className="text-center py-10 text-slate-300 text-xs font-bold uppercase border-2 border-dashed border-slate-100 rounded-xl">No history found</div>
                            ) : (
                                historyLogs.map(log => {
                                    ...
                                    return (
                                        <div key={log.id} className="flex gap-4 group">
                                            ...
                                            <div className="flex flex-col items-center">
                                                <div className={`w-2 h-2 rounded-full mt-1.5 ${isIn ? 'bg-emerald-400' : (isOut ? 'bg-red-400' : 'bg-blue-400')}`}/>
                                                ...
                                                <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex items-center gap-2">
                                                            <span className={`text-[10px] font-black uppercase px-1.5 py-0.5 rounded ${isIn ? 'bg-emerald-100 text-emerald-600' : (isOut ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600')}`}>
                                                                {log.type}
                                                            </span>
                                                            <span className="text-[10px] text-slate-400 font-bold">{new Date(log.date).toLocaleString()}</span>
                                                        </div>
                                                        <div className={`font-mono font-bold text-sm ${isIn ? 'text-emerald-600' : (isOut ? 'text-red-500' : 'text-blue-500')}`}>
                                                            {isIn ? '+' : ''}{log.qtyChange}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                                                        <div>
                                                            ...
                                                            <span className="font-bold text-slate-700">{formatMoney(Number(log.unitCost))}</span>
                                                        </div>
                                                        <div className="text-right">
                                                            <span className="text-slate-400 block mb-0.5">Reference</span>
                                                            <span className="font-bold text-slate-600 bg-white px-1.5 py-0.5 rounded border border-slate-100 truncate inline-block max-w-[80px]" title={log.refId}>{log.refId}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
