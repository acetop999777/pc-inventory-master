...
import { Scan, Box, Trash2, Loader2 } from 'lucide-react';
import {
  generateId,
  ALL_CATS,
  apiCall,
...
export default function IntakeNode({ data, refresh, notify, log: _log }: Props) {
  ...
  const handleScan = async (e: React.FormEvent) => {
    ...
    // B. 检查现有库存 (完全匹配 SKU)
    const match = data.inv.find(
      (i) => (i.sku && i.sku === code) || (i.keyword && i.keyword === code),
    );
    if (match) {
      setBatch((p) => [
        { ...match, qtyInput: 1, costInput: match.cost, isMatch: true } as StagedItem,
        ...p,
      ]);
      setScanVal('');
      return;
  ...
  const parseNewegg = () => {
    try {
      ...
      if (finalBatch.length === 0) notify('No items found', 'error');
        ...
        setNeweggTxt('');
        notify(
          `Parsed ${finalBatch.length} items. Matched ${finalBatch.filter((i) => i.isMatch).length} existing.`,
        );
      }
    } catch (e) {
      console.error(e);
      ...
      <div className="space-y-6 h-full flex flex-col">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex-shrink-0">
          ...
          <form onSubmit={handleScan} className="relative">
            ...
            <input
              autoFocus
              className="w-full bg-slate-50 pl-12 pr-4 py-3.5 rounded-xl font-bold text-sm outline-none focus:ring-2 focus:ring-blue-100 transition-all"
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex-1 flex flex-col min-h-0">
          ...
          <textarea
            className="flex-1 bg-slate-50 rounded-xl p-4 text-xs font-mono mb-4 outline-none resize-none focus:ring-2 focus:ring-blue-100 transition-all leading-relaxed"
            placeholder="Paste Newegg Order Summary..."
            value={neweggTxt}
            onChange={(e) => setNeweggTxt(e.target.value)}
          />
          <button
            onClick={parseNewegg}
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 flex flex-col h-full min-h-0">
        <div className="flex justify-between items-center mb-6 flex-shrink-0">
          ...
          {batch.length > 0 && (
            <button
              onClick={commit}
              className="bg-slate-900 text-white px-6 py-2 rounded-xl font-bold text-xs uppercase shadow-lg active:scale-95 transition-all"
            >
              Commit All
            </button>
          )}
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-2 no-scrollbar pb-10">
          {batch.map((item, i) => {
            ...
            return (
              <div
                key={i}
                className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3 group hover:border-blue-200 transition-colors animate-in slide-in-from-bottom-2 relative"
              >
                <div className="flex justify-between items-start relative">
                  ...
                  <button onClick={() => setBatch(batch.filter((_, idx) => idx !== i))}>
                    <Trash2
                      size={16}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <select
                    className="text-[10px] font-bold bg-slate-50 text-slate-500 rounded-lg px-2 py-1 outline-none uppercase"
                    value={item.category}
                    onChange={(e) => {
                      const n = [...batch];
                ...
                <div className="grid grid-cols-2 gap-4 mt-1">
                  <div className="bg-slate-50 rounded-xl p-2 flex items-center justify-between px-3">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...batch];
                        n[i].qtyInput = parseInt(e.target.value) || 0;
                        setBatch(n);
                      }}
                    />
                  </div>
                  <div className="bg-slate-50 rounded-xl p-2 flex items-center justify-between px-3">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = [...batch];
                        n[i].costInput = parseFloat(e.target.value) || 0;
                        setBatch(n);
                      }}
                    />
                  </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  />
                  {item.isMatch && (
                    <span className="text-[9px] text-slate-300 font-bold ml-2">ID_LINKED</span>
                  )}
                </div>
              </div>
            );
