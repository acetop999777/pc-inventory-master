...
export const StagingArea: React.FC<Props> = ({ batch, inventory, setBatch, onCommit }) => {
    ...
    return (
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 flex flex-col h-full min-h-0" onClick={() => setActiveDrop(null)}>
            <div className="flex justify-between items-center mb-6 flex-shrink-0">
                <h3 className="font-black uppercase text-xs text-slate-400 tracking-widest">Staging ({batch.length})</h3>
                {batch.length > 0 && (
                    <button 
                        onClick={onCommit} 
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 no-scrollbar pb-10">
                {batch.map((item, i) => {
                    const suggestions = activeDrop === i ? inventory.filter(inv => {
                        const searchStr = item.name.toLowerCase();
                        return inv.name.toLowerCase().includes(searchStr) || (inv.keyword && searchStr.includes(inv.keyword.toLowerCase()));
                    }).slice(0, 5) : [];

                    return (
                        <div key={i} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3 group hover:border-blue-200 transition-colors relative animate-in slide-in-from-bottom-2 duration-300">
                                        ...
                                        <div className="absolute top-full left-0 w-full bg-white shadow-xl rounded-xl border border-slate-100 z-50 mt-2 p-1 overflow-hidden animate-in fade-in zoom-in-95">
                                            {suggestions.map(s => (
                                                <div key={s.id} onClick={(e) => { e.stopPropagation(); matchToLocal(i, s); }} className="px-3 py-2 text-xs hover:bg-slate-50 rounded-lg cursor-pointer flex justify-between items-center">
                                                    ...
                                                    <div className="text-slate-400 font-mono text-[10px] whitespace-nowrap bg-slate-100 px-1.5 py-0.5 rounded">LINK</div>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                </div>
                                <button onClick={() => setBatch(batch.filter((_, idx) => idx !== i))} className="ml-2 text-slate-300 hover:text-red-500 transition-colors">
                                    <Trash2 size={16}/>
                                </button>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2 items-center">
                                <span className={`text-[10px] font-black px-2 py-1 rounded-lg uppercase ${item.isMatch ? 'bg-emerald-50 text-emerald-600' : (item.isApi ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500')}`}>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                />
                                {item.isMatch && <span className="text-[9px] text-slate-300 font-bold ml-2 whitespace-nowrap">ID_LINKED</span>}
                            </div>
                        </div>
                    );
                })}
