        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div className="flex justify-between items-center relative mb-8 px-2">
                ...
                {statusOptions.map((s, i) => (
                    <div key={s} onClick={() => update('status', s)} className="group flex flex-col items-center gap-2 cursor-pointer">
                        <div className={`w-4 h-4 rounded-full border-2 transition-all ${i <= statusOptions.indexOf(data.status) ? 'bg-blue-600 border-blue-600' : 'bg-white border-slate-300 group-hover:border-blue-300'}`}/>
                        <span className={`text-[8px] font-bold uppercase transition-colors ${data.status===s ? 'text-blue-600' : 'text-slate-300'}`}>{s.split(' ')[0]}</span>
                    </div>
                ))}
            </div>

            <div className="space-y-4">
                {/* 仅保留 Order Date 和 Delivery Date */}
                ...
                <div className="pt-4 border-t border-slate-50">
                    ...
                    <div className="flex items-center gap-2 mb-3 cursor-pointer" onClick={() => update('isShipping', !data.isShipping)}>
                        <div className={`w-9 h-5 rounded-full p-0.5 transition-colors ${data.isShipping?'bg-blue-600':'bg-slate-200'}`}><div className={`w-4 h-4 bg-white rounded-full transition-transform shadow-sm ${data.isShipping?'translate-x-4':''}`}/></div>
