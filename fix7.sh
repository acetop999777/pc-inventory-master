...
cat > client/src/app/saveQueue/SaveQueue.ts <<'EOF'
...
git add client/src/app/saveQueue/SaveQueue.ts
git commit -m "phase3.2: make SaveQueue.getSnapshot stable for useSyncExternalStore" || true

TAG="phase3_2-$(date +%Y%m%d)"
if git rev-parse "$TAG" >/dev/null 2>&1; then
  TAG="${TAG}b"
fi
git tag -a "$TAG" -m "phase3.2: fix React #185 loop"
